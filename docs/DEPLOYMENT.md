...
| `ATTACHMENT_MAX_PER_COMMENT` | ❌ | `3` | Максимальное количество вложений на комментарий |
| `ATTACHMENT_ALLOWED_PATTERNS` | ❌ | `*` | Разрешённые типы файлов (glob-маски через запятую). `*` = разрешить все |
| `ATTACHMENT_TOTAL_SIZE` | ❌ | `0` | Общий лимит хранилища вложений на сервер в МБ. `0` = безлимит |
| `ATTACHMENT_USER_MAX_SIZE` | ❌ | `512` | Лимит хранилища вложений на пользователя в МБ. `0` = безлимит |
